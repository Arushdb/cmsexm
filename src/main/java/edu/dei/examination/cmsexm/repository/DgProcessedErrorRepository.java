...
public interface DgProcessedErrorRepository extends JpaRepository<DgExtract, Integer> {
	...
	@Query(name = "updatedgerrorstatus" ,nativeQuery = true,value =
			"UPDATE dg_error  SET status = 'P' WHERE status = 'E'") 
			@Modifying
			int updatedgerrorstatus();
	...
	@Query(name = "getstudentlist" ,nativeQuery = true,value =
		...
		+   "   if(s.program_id = '0001009','T Theory  P Practical',' ') remarks,"
		+   "   if(s.program_id = '0001009',concat('T',' ',s.theory_sgpa,'  ','P',' ',s.practical_sgpa),s.sgpa) as sgpa, "
		...
		+	" sp.current_semester ,de.abc_id ,de.aadhaar_name as AADHAAR_NAME ,sa.theory_sgpa,sa.practical_sgpa,sp.registered_from_session,"
		+   "if(sp.current_semester = pch.semester_code and sp.program_status = 'PAS' ,sp.theory_cgpa,'') as theory_cgpa,"
		+   "if(sp.current_semester = pch.semester_code and sp.program_status = 'PAS' ,sp.practical_cgpa,'') as practical_cgpa from "
		...
		+	"          ss.semester_start_date = srsh.session_start_date  "
		+   " join exam_live.dg_error de on de.roll_number = sp.roll_number "
