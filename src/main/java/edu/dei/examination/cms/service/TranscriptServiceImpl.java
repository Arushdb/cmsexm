...
import edu.dei.examination.cms.repository.TranscriptRepository;
...
//	private final String WATERMARK_IMAGE_PATH = "/images/DEI-WATERMARK.jpg";
	...
	public void generateTranscriptPdf(Transcript transcript, OutputStream outputStream) {
		...
		try {
			...
			// FileOutputStream("document_with_page_numbers.pdf"));
			...
			PdfPCell titleCell = new PdfPCell(
					new Phrase("OFFICIAL TRANSCRIPTS OF MARKS", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 11)));
			...
			PdfPCell serialCell = new PdfPCell(
					new Phrase("No./Exam/" + serialNumber, FontFactory.getFont(FontFactory.HELVETICA, 10)));
			...
			PdfPCell dateCell = new PdfPCell(
					new Phrase("Dated: " + currentDate, FontFactory.getFont(FontFactory.HELVETICA, 10)));
			...
			Paragraph desc1 = new Paragraph(
					"The Institute follows a credits system and evaluation by letter grading on an 11-point scale. The Letter Grades, corresponding Grade Point Values, and their achievement level are given below:\n\n",
			...
			desc2.add(new Chunk(
					" is the total number of credits earned by the student in that Semester as the case may be)\n",
					font));
			desc2.add(new Chunk(
					"Numerically, all GPA computations are expressed up to three decimal places. This ensures:\n",
					font));
			desc2.add(new Chunk(
			...
			desc2.add(new Chunk(
					"Award of Divisions: The minimum CGPA for different divisions is given in the following table.",
					font));
		...
		//	createTranscriptTables(transcript.getRoll_number(), document); // Call the modified table creation method
			...
			paragraph.add(
					new Chunk(transcript.getStudent_first_name(), FontFactory.getFont(FontFactory.HELVETICA_BOLD, 12)));
			...
			paragraph.add(new Chunk(
					" at DEI. This is a regular course conducted as per requirements prescribed by the Dayalbagh Educational Institute (Deemed to be University), Dayalbagh, Agra. He/She has successfully completed the course with all credits required for the award of the degree without any backlog.\n\n"));
			...
			// Paragraph ARTitle = new Paragraph("Assistant Registrar (Exam.)",
			// FontFactory.getFont(FontFactory.HELVETICA, 12));
			// ARTitle.setAlignment(Paragraph.ALIGN_RIGHT);
			// document.add(ARTitle);

			// Create a table with two columns (one for the date on the left and one for AR
			// title on the right)
			...
			PdfPCell arTitleCell = new PdfPCell(
					new Phrase("Assistant Registrar (Exam.)", FontFactory.getFont(FontFactory.HELVETICA, 12)));
	...
	private void addGradeTable(Document document) throws DocumentException {
		...
		// Grade data
		String[][] gradeData = { { "A", "10", "Outstanding" }, { "D", "4", "Below Average" },
				{ "A-", "9", "Excellent" }, { "D-", "3", "Just Pass" }, { "B", "8", "Very Good" },
				{ "E", "2", "Extension Grade" }, { "B-", "7", "Good" }, { "E-", "1", "Extension Grade" },
				{ "C", "6", "Above Average" }, { "F", "0", "Fail" }, { "C-", "5", "Average" }, { "", "", "" } };
	...
	private void addDivisionTable(Document document) throws DocumentException {
		...
		String[][] divisionData = { { "First with Distinction", "8.500" }, { "First", "6.000" }, { "Second", "4.500" },
				{ "Pass", "3.000" } };
	...
	public boolean rollNumberExists(String rollNumber) {
		Integer count = transcriptRepository.countByRollNumber(rollNumber);
		return count != null && count > 0;
	}

	public boolean isStudentPassed(String rollNumber) {
		String status = transcriptRepository.getProgramStatus(rollNumber);
		return "PAS".equals(status);
	}

	public Transcript getTranscriptByRollNumber(String rollNumber) {
		return transcriptRepository.getTranscriptByRollNumber(rollNumber);
	}

	public String generateSerialNumber(String rollNumber) {
		// Query to check if the serial number already exists for the student
		String queryCheck = "SELECT transcript_number FROM student_program WHERE roll_number = ? group by roll_number";
		String serialNumber = cmsJdbcTemplate.queryForObject(queryCheck, new Object[] { rollNumber }, String.class);
		...
		if (serialNumber != null) {
			...
			String queryLastSerial = "SELECT value FROM system_values WHERE code = 'TRNCPT'";
			Integer lastNumber = cmsJdbcTemplate.queryForObject(queryLastSerial, Integer.class);
			...
			String updateSerial = "UPDATE system_values SET value = ? WHERE code = 'TRNCPT'";
			cmsJdbcTemplate.update(updateSerial, newSerialNumber);

			// Save the new serial number in the student_program table
			String formattedSerialNumber = String.format("%06d", newSerialNumber);
			String updateStudentSerial = "UPDATE student_program SET transcript_number = ? WHERE roll_number = ?";
			cmsJdbcTemplate.update(updateStudentSerial, formattedSerialNumber, rollNumber);
	...
	// String sql = "SELECT COUNT(*) FROM cms_live.student_program WHERE roll_number
	// = ? AND program_status = 'PAS'";
	// Integer count = jdbcTemplate.queryForObject(sql, new Object[]{rollNumber},
	// Integer.class);
	// return count != null && count > 0;
	// }
	...
	// String query = "SELECT COUNT(*) FROM cms_live.student_program WHERE
	// roll_number = ? ORDER BY program_completion_date DESC LIMIT 1";
	// Integer count = jdbcTemplate.queryForObject(query, new Object[]{rollNumber},
	// Integer.class);
	// return count != null && count > 0;
	// }
	...
	// String query = "SELECT program_status FROM cms_live.student_program WHERE
	// roll_number = ? ORDER BY program_completion_date DESC LIMIT 1";
	// String status = jdbcTemplate.queryForObject(query, new Object[]{rollNumber},
	// String.class);
	...
	// String sql = "SELECT t1.FromDate, t1.ToDate, t2.duration, t2.medium,
	// t2.roll_number, t2.student_first_name, t2.program_name, t2.enrollment_number,
	// t2.date_of_birth, t2.cgpa " +
	// "FROM ( " +
	// " SELECT srsh.roll_number, MIN(SUBSTRING(sp.registered_from_session, 1, 4))
	// AS FromDate, " +
	// " MAX(SUBSTRING(sp.passed_to_session, 1, 4)) AS ToDate " +
	// " FROM cms_live.student_registration_semester_header srsh " +
	// " JOIN cms_live.program_course_header pch ON srsh.program_course_key =
	...
	// " AND pch.specialization_id = sp.specialization_id AND pch.branch_id =
	// sp.branch_id " +
	// " JOIN cms_live.program_master pm ON pm.program_id = sp.program_id " +
	// " JOIN cms_live.student_master sm ON sm.enrollment_number =
	// sp.enrollment_number " +
	// " WHERE sp.program_status IN ('PAS', 'SWT') AND srsh.roll_number = ? " +
	// ") AS t1 " +
	// "JOIN ( " +
	// " SELECT pm.months_duration_in_english AS duration, 'ENGLISH' AS medium,
	// srsh.roll_number, sm.student_first_name, pm.program_name,
	// sp.enrollment_number, sm.date_of_birth, sp.cgpa " +
	// " FROM cms_live.student_registration_semester_header srsh " +
	// " JOIN cms_live.program_course_header pch ON srsh.program_course_key =
	// pch.program_course_key " +
	// " JOIN cms_live.student_program sp ON srsh.roll_number = sp.roll_number " +
	...
	// return jdbcTemplate.queryForObject(sql, new Object[]{roll_number,
	// roll_number}, new RowMapper<Transcript>() {
	// @Override
